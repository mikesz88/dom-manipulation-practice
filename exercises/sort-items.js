...
const sortData = (sort) => {
  ...
  if (sort === 'desc') {
    sortedData = Array.from(allItems).sort((a, b) => b.id - a.id);
  } else {
    sortedData = Array.from(allItems).sort((a, b) => a.id - b.id);
  }
  container.innerHTML = '';
...
// Your code goes here...
Array.from(sortBtn).forEach((item) => {
  item.addEventListener('click', () => {
    if (item.dataset.sortdir === 'desc') {
      sortData('desc');
    } else {
      sortData('asc');
    }
